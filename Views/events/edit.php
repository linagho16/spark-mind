...
<div class="container">
    ...
    <form method="post" action="index.php?route=events.edit&id=<?= $event['id'] ?>" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <textarea id="description" name="description" 
                      class="<?= isset($errors['description']) ? 'error-field' : '' ?>"><?= htmlspecialchars($event['description'] ?? '') ?></textarea>
        ...
        <div class="form-group">
            <label for="date_event" class="required">Date (YYYY-MM-DD)</label>
            <input type="date" id="date_event" name="date_event" 
                   value="<?= htmlspecialchars($event['date_event'] ?? '') ?>" 
        ...
        <div class="form-group">
            ...
            <input type="number" id="prix" name="prix" step="0.01" 
                   value="<?= htmlspecialchars($event['prix'] ?? '0.00') ?>" 
            ...
            <select id="categorie_id" name="categorie_id" class="<?= isset($errors['categorie_id']) ? 'error-field' : '' ?>">
                ...
                <?php foreach($categories as $categorie): ?>
                    <option value="<?= $categorie['id'] ?>" 
                        <?= (isset($event['categorie_id']) && $event['categorie_id'] == $categorie['id']) ? 'selected' : '' ?>
            ...
            <?php if(!empty($event['image'])): ?>
                <div style="margin-top: 10px;">
                    ...
                    <img src="/evenement/uplodes/events/<?= htmlspecialchars($event['image']) ?>" 
                         ...
                         style="max-width: 200px; max-height: 150px; border-radius: 8px; margin-top: 5px;">
                </div>
            <?php endif; ?>
        </div>
